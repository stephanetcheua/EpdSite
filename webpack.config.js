...
module.exports = function (options, webpackOptions) {
    ...
    if (options.prod && options.aot) {
        console.log(`Running build for ${options.client ? 'client' : 'server'} with AoT and Prod Compilation`);
    } else if (options.aot) {
        console.log(`Running build for ${options.client ? 'client' : 'server'} with AoT Compilation`);
    }
    
    if (options.prod) {
        console.log('Deactivate sourceMaps');
        delete(commonPartial.devtool);
    }
